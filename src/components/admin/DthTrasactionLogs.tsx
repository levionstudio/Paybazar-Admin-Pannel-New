...
import { Loader2, RefreshCw, Eye, Download, Search, Calendar, Check, ChevronsUpDown, Tv } from "lucide-react";
...
interface DTHRechargeTransaction {
...
const DTHRechargeTransactionPage = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      if (!adminId || !token) {
        console.log("Skipping users fetch - no admin ID or token");
        return;
      }
      ...
      try {
        ...
        if (response.data.status === "success" && response.data.data) {
          ...
          console.log("Retailers count:", usersList.length);
          setUsers(usersList);
        } else {
          console.warn("No retailers found in response");
          setUsers([]);
        }
      } catch (error: any) {
        console.error("=== Error Fetching Retailers ===");
        ...
        console.log("=== Retailers Fetch Completed ===");
      }
    };
  ...
  const fetchAllTransactions = useCallback(async () => {
    ...
    console.log("=== Fetching All DTH Transactions ===");
    setLoadingAllTransactions(true);

    try {
      const url = `${import.meta.env.VITE_API_BASE_URL}/dth_recharge/get/admin`;
      console.log("Fetching from URL:", url);
      ...
      const list: DTHRechargeTransaction[] =
        response.data?.data?.recharges ?? [];

      console.log("Parsed recharges list:", list);
      ...
      console.log("=== All Transactions Fetch Completed ===");
    }
  }, [token]);
  ...
  const fetchUserTransactions = useCallback(async () => {
    ...
    console.log("=== Fetching User DTH Transactions ===");
    console.log("Selected User ID:", selectedUserId);
    setLoadingUserTransactions(true);
    
    try {
      const url = `${import.meta.env.VITE_API_BASE_URL}/dth_recharge/get/${selectedUserId}`;
      console.log("Fetching from URL:", url);
      ...
      const list: DTHRechargeTransaction[] = response.data?.data?.recharges || [];
      ...
      console.log("=== User Transactions Fetch Completed ===");
    }
  ...
  const exportToExcel = async (isUserData: boolean) => {
    ...
    try {
      ...
      console.log("Data to export:", allData.length, "transactions");
      ...
      XLSX.utils.book_append_sheet(wb, ws, "DTH Recharge Transactions");

      const timestamp = new Date().toISOString().slice(0, 10);
      const filename = isUserData 
        ? `Retailer_${selectedUserId}_DTH_Recharge_Transactions_${timestamp}.xlsx`
        : `All_DTH_Recharge_Transactions_${timestamp}.xlsx`;
  ...
  const renderTransactionTable = (
    transactions: DTHRechargeTransaction[],
    currentPage: number,
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">DTH Recharge Transactions</h1>
          <p className="text-gray-600 mt-1">
            View and manage DTH recharge transaction history
          </p>
        </div>
      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      placeholder="Search by customer ID, operator..."
                      value={allSearchTerm}
                      ...
                      <CommandGroup className="max-h-64 overflow-auto">
                        {users.map((user) => (
                          <CommandItem
                            ...
                            onSelect={() => {
                              console.log("Selected retailer:", user.retailer_id, user.retailer_name);
...
export default DTHRechargeTransactionPage;